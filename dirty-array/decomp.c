...
ulong decomposeNumber(const ulong *primes, ulong *factors, size_t primeCount, ulong number) {
    ulong p;
    ulong j = 0;
...
void writeFactorsToFile(ulong *primes, ulong *factors, size_t primeCount, ulong number, ulong maxFactorIndex, FILE* file) {
    bool first = TRUE;
...
void decomp(ulong *primes, size_t primeCount, size_t tableSize, const char* path) {
    ...
    for (ulong i = 0; i < tableSize; i++) {
        ...
        ulong maxFactorIndex = decomposeNumber(primes, factors, primeCount, i);
        //Saving to file
        writeFactorsToFile(primes, factors, primeCount, i, maxFactorIndex, file);
        registerProgress();
    }
    stopProgressReport();
    fclose(file);
